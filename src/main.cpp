...
int main(int argc, char *argv[]) {
    ...
    Camera camera = Camera(Vec(0, 0, 2.5), Vec(0,0,1), Vec(0, 1, 0), 320, 240, mono_eq);     // Create camera
    ...
    scene.add( dynamic_cast<Object*>(new Sphere(Vec(0,0,0), 1, Material(EMIT, Spectrum("../data/spike700.csv"), Spectrum(0.0)))) );
    scene.add( dynamic_cast<Object*>(new Cylinder(Vec(0,0,0), Vec(0,0,1), 5, 20, Material())));
